...
backend server1 { # Define one backend
  ...
  .host = "158.69.24.20";    # IP or Hostname of backend
  .port = "8008";           # Port Apache or whatever is listening
...
sub vcl_recv {
  ...
  if (req.http.host ~ "^channelindependent.com") {
    return (synth (750, ""));
  }
  ...
  set req.http.Cookie = regsuball(req.http.Cookie, "__unam=[^;]+(; )?", "");
...
sub vcl_synth {
  if (resp.status == 750) {
    set resp.status = 301;
    set resp.http.Location = "http://www.channelindependent.com" + req.url;
    return(deliver);
  }
}
...
sub vcl_deliver {
  ...
  #set resp.http.Access-Control-Allow-Origin = "http://1138431148.rsc.cdn77.org http://www.channelindependent.com";
  set resp.http.Access-Control-Allow-Origin = "http://www.channelindependent.com";
